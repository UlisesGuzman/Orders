...
public final class DBOperations {
    ...
    public Cursor getTitles(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.TITLE);
        return  database.rawQuery(sql, null);
    }

    public Cursor getTitleById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.TITLE,
                OrderContract.Titles.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }
    ...
    public String insertTitle(Title title){
        SQLiteDatabase database = db.getWritableDatabase();
        String idTitle = OrderContract.Titles.generateIdTitle();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Titles.ID, idTitle);
        values.put(OrderContract.Titles.TITLE, title.getTitle());
        values.put(OrderContract.Titles.FROMDATE, title.getFromDate());
        values.put(OrderContract.Titles.TODATE, title.getToDate());
        database.insertOrThrow(OrderDB.Tables.TITLE, null, values);
        return idTitle;
    }
    ...
    public boolean updateTitle(Title title){
        ...
        values.put(OrderContract.Titles.FROMDATE, title.getFromDate());
        values.put(OrderContract.Titles.TODATE, title.getToDate());
        String whereClause = String.format("%s=?", OrderContract.Titles.ID);
        String[] whereArgs = {title.getIdEmpno()};
        int result = database.update(OrderDB.Tables.TITLE,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteTitle(String idTitle){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.Titles.ID + "=?";
        String[] whereArgs = {idTitle};
        int result =  database.delete(OrderDB.Tables.TITLE,
                whereClause, whereArgs);
        return result > 0;
    }
